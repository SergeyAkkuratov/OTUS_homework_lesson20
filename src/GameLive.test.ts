...
describe("GameLive class tests", () => {
    ...
    function setEventProperty(event: Event, propertyName: string, propertyValue: HTMLElement){
        Object.defineProperty(event, propertyName, {value: propertyValue, configurable: true});
    }
    ...
    it("Constructor tests", () => {
        ...
        const element = rootElement.querySelector(`cell[coor-x = '0'][coor-y = '0']`) as HTMLElement
        element.click();
        cell.onClick();
    ...
    it("Check showGrid function", () => {
        ...
        getGrid(10,10).forEach(row => row.forEach(cell => div.appendChild(cell.cellElement)));
    ...
    it("Check markGrid (invisible) function", () => {
        const grid = getGrid(5, 5);
        for(let i=1;i<4;i+=1){
            (rootElement.querySelector(`cell[coor-x = '${i}'][coor-y = '1']`) as HTMLElement).click();
            grid[1][i].onClick();
        }
    ...
    it("Check getCellNeighbours function", () => {
        ...
        expect(gameLive.getCellNeighbours(new Cell(0,0))).toEqual(expect.arrayContaining(expected));
    })
    ...
    it.each([
         ...
         [0, 5, 0, 0],
         [0, -3, 0, 2],
    ])("Checks getCell function", (x: number, y: number, expectX: number, expectY: number) =>{
    ...
    it("Check showFigure function", () => {
        ...
        setEventProperty(eventMouseOut, "relatedTarget", rootElement.querySelector(`cell[coor-x = '1'][coor-y = '0']`) as HTMLElement);
        rootElement.dispatchEvent(eventMouseOut);
        
        expect(gameLive.grid).toStrictEqual(getGrid(5,5));
        
        setEventProperty(eventMouseOver, "target", rootElement.querySelector(`cell[coor-x = '0'][coor-y = '0']`) as HTMLElement);
        rootElement.dispatchEvent(eventMouseOver);

        expect(gameLive.grid).toStrictEqual(grid);


        setEventProperty(eventMouseOver, "target", rootElement.querySelector(`cell[coor-x = '1'][coor-y = '0']`) as HTMLElement);
        rootElement.dispatchEvent(eventMouseOver);
